...
if __name__ == '__main__':
    ...
    DRIVER.find_element_by_name(LOGIN_INFO['login_id_name']).send_keys(LOGIN_INFO['login_id'])
    DRIVER.find_element_by_name(LOGIN_INFO['password_name']).send_keys(LOGIN_INFO['password'])
    DRIVER.find_element_by_xpath(LOGIN_INFO['submit_path']).click()
    ...
    if len(CATEGORY_LIST) == 0:
        ...
        for data in DATA_LIST:
            if data.get_attribute(ATTRIBUTE).startswith(PATTERN):
                ...
                CATEGORY_LINK_LIST.append([data.get_attribute(ATTRIBUTE), link_text])
    ...
    for category_link in CATEGORY_LIST:
        ...
        category_page = DRIVER.get(category_link[0])

        try:
            lbl_count = DRIVER.find_element_by_xpath('//span[@id="MainContent_lblCount"]').text
        except NoSuchElementException:
            logprint('件数が見つかりません')

        lbl_count = bytes(lbl_count, 'utf-8').decode('utf-8')
        ...
        while True:
            ...
            try:
                data_list = DRIVER.find_elements_by_xpath('//div[@id="title"]/h2/a')
            except NoSuchElementException:
            ...
            webcd_list = []
            for data in data_list:
                if data.get_attribute('href') is not None:
                    webcd_list.append(data.get_attribute('href'))
            ...
            try:
                next_link = DRIVER.find_element_by_xpath(
                    active_element_xpath + '/following-sibling::*')
            except NoSuchElementException:
                print('NoSuchElementException')
                break
            ...
            click_text = next_link.text
            # アクティブな要素の次の要素をクリックする
            try:
                next_link.click()
            except NoSuchElementException:
                print('NoSuchElementException')
                logprint(click_text + 'のクリックに失敗しました。')
            else:
    ...
    for product_row in PRODUCT_URL:
        ...
        # lbl_category = DRIVER.find_elements_by_xpath(
        lbl_category = DRIVER.find_elements_by_xpath(
        ...
        # 07 品名1: 商品名に該当するもの全て
        try:
            lbl_goods_name = DRIVER.find_element_by_xpath(
        ...
        try:
            maker_name = DRIVER.find_element_by_xpath(
        ...
        try:
            cost_text = DRIVER.find_element_by_xpath(
        ...
        # オプションを取得する。
        option_list = []
        option_list = DRIVER.find_elements_by_xpath(
        ...
        try:
            lbl_unit = DRIVER.find_element_by_xpath("//span[@id='MainContent_lblUnit']").text
        except NoSuchElementException:
            lbl_unit = ''
        else:
            pass
        ...
        # 27 JAN: JANコード
        try:
            jancd = DRIVER.find_element_by_xpath("//span[@id='MainContent_lblJANCD']").text
        except NoSuchElementException:
            jancd = ''
        else:
            pass
        ...
        # 注意: r-commmentのmは3つ
        try:
            r_comment = DRIVER.find_element_by_xpath(
                "//div[@id='r-commment']/p"
        ...
        #               タグは含むが、<span>タグは削除
        try:
            r_spec = DRIVER.find_element_by_xpath(
                "//div[@id='r-spec']/p"
            ).get_attribute('outerHTML')

            spec = DRIVER.find_element_by_xpath(
        ...
        # 商品画像
        image_tag_list = DRIVER.find_elements_by_xpath("//div[@id='up']/img")
        up_image_list = []
        for image_tag in image_tag_list:
            up_image_list.append(image_tag.get_attribute('src'))
        ...
        # アイコン画像
        image_tag_list = DRIVER.find_elements_by_xpath("//div[@id='r-icon']/img")
        icon_image_list = []
        for image_tag in image_tag_list:
            icon_image_list.append(image_tag.get_attribute('src'))
        ...
        # 複数ある場合は半角スペースで区切る
        category_list = []
        r_category_list = DRIVER.find_elements_by_xpath(
